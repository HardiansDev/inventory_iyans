...
@section('title')
    <title>Sistem Inventory Iyan | Posisi Pegawai</title>
@endsection
...
@section('content')
    <section class="mb-6 rounded-lg bg-white p-6 shadow-sm">
        <div class="mx-auto max-w-7xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Manajemen Jabatan Pegawai</h1>
                <p class="mt-1 text-sm text-gray-500">Kelola data jabatan dan gaji pokok pegawai</p>
            </div>
        </div>
    </section>
        ...
        <div class="overflow-x-auto rounded-lg border border-gray-200 shadow">
            <table class="min-w-full divide-y divide-gray-200 bg-white text-sm text-gray-700">
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-4 py-6 text-center text-gray-500">Tidak ada data.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="positionModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-50">
        <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-lg">
            <h2 class="mb-4 text-lg font-semibold text-gray-800">Tambah Jabatan</h2>
            <form action="{{ route('position.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label for="name" class="block text-sm font-medium text-gray-700">Nama Jabatan</label>
                    <input type="text" name="name" required
                        class="w-full mt-1 px-3 py-2 border rounded focus:ring focus:border-blue-500" />
                </div>
                <div class="mb-4">
    ...
    <div id="editPositionModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-50">
        <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-lg">
            <h2 class="mb-4 text-lg font-semibold text-gray-800">Edit Jabatan</h2>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-50">
        <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-lg">
            ...
            <p class="mb-6 text-gray-700">Yakin ingin menghapus jabatan <span id="positionName" class="font-bold"></span>?
            </p>
