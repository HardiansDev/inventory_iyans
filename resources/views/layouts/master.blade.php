...
>
    ...
    >
        <button
            id="sidebar-toggle"
            class="p-2 text-gray-800 focus:outline-none"
        >
        ...
        @if (Auth::user()->role === 'superadmin')
            <div class="relative ml-4">
                <button
                    ...
                    class="relative p-2 text-gray-800 focus:outline-none"
                >
                ...
                <div
                    id="notificationMenu"
                    class="absolute right-0 z-50 mt-2 hidden w-64 rounded-md bg-white shadow-lg"
                >
                    <div class="py-2 text-sm text-gray-700">
                        @if ($pendingRequests > 0)
                            @foreach (\App\Models\ProductIn::where('status', 'menunggu')->latest()->take(5)->get() as $notif)
                                <a
                                    href="{{ route('product.confirmation', $notif->id) }}"
        ...
        <div class="relative ml-auto inline-block text-left">
            ...
            <div
                id="userMenu"
                class="absolute right-0 z-50 mt-2 hidden w-48 rounded-md bg-white shadow-lg"
            >
                <div class="py-1">
                    <a
                        href="#"
    ...
    @if (session('success') || session('error') || session('info') || session('warning'))
        <div
            x-data="{ show: true, type: '{{ session('success') ? 'success' : (session('error') ? 'error' : (session('warning') ? 'warning' : 'info')) }}' }"
            x-show="show"
            ...
            class="fixed right-5 top-5 z-[9999] w-full max-w-xs rounded-md px-4 py-3 shadow-lg"
            role="alert"
        >
