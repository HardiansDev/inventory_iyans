...
@section('content')
    <section class="max-w-3xl mx-auto mt-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8">
        <h1 class="text-3xl font-bold mb-8 text-gray-900 dark:text-gray-100">Edit Bahan Baku</h1>
               ...
               dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"
                        ...
                        @foreach ($satuans as $satuan)
                            <option value="{{ $satuan->id }}"
                                {{ $bahanBaku->satuan_id == $satuan->id ? 'selected' : '' }}>
                                {{ $satuan->nama_satuan }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full rounded-lg border border-gray-300 dark:border-gray-600
                                 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100
                                 px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none
                                 transition @error('description') border-red-500 @enderror">{{ old('description', $bahanBaku->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
